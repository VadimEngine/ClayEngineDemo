...
LightingSceneGUI::LightingSceneGUI(LightingScene& theScene)
: mScene_(theScene) {
...
LightingSceneGUI::~LightingSceneGUI() { }

void LightingSceneGUI::buildImGui() {
...
void LightingSceneGUI::buildCameraSection() {
    ImGui::Text("Camera");
...
void LightingSceneGUI::buildEntitySection() {
    ImGui::Text("Entities");
    ...
    if (mSelectedEntityIndex_ < mScene_.getEntities().size()) {
        ...
        // color
        if (dynamic_cast<LightEntity*>(selectedEntity)) {
            LightEntity* light = dynamic_cast<LightEntity*>(selectedEntity);
            // selectedEntity points to a LightEntity
            float lightColor[4] = {
                light->getColor().r,
                light->getColor().g,
                light->getColor().b,
            ...
            if (ImGui::ColorEdit4("Color##Light", lightColor)) {
                light->setColor({
                    lightColor[0],
                    lightColor[1],
                    lightColor[2],
